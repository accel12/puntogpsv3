...
const ShareDesktopPage = ({ setLinkWzz, link, setLink }) => {
  const navigate = useNavigate();
  const classes = useSettingsStyles();
  const t = useTranslation();
  const device = useSelector((state) => state.session.user);
  ...
  const handleShare = useCatchCallback(async () => {
    ...
    if (response.ok) {
      ...
      setLinkWzz(`${window.location.origin}?token=${token}`);
    } else {
...
export default ShareDesktopPage;
